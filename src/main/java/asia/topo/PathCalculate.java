...
public class PathCalculate {
    ...
    public Path calculatePath(DefaultDirectedWeightedGraph ggg, int Source, int Dest) {
        ...
        pa.nodes = ksp.getPaths(Final.graph.vertex.get(Dest));
        return pa;
    }
...
//        int pathNum=0;         //记录遍历完k条路径后的不可用的路径数
        if (bandwave > 80)
            ...
            for (int x=0; x< p.nodes.size();x++){
                ...
                do {
                    ...
                    AccessEdge edge = edgeList.get(y);
                    int srcDomain = edge.getSource().domain;
                    int dstDomain = edge.getDest().domain;
                    if (srcDomain!=dstDomain) {
                        //如果在不同域，分成两个单域来分配资源
                        edgedomain1=edge.getSource().identifier;//第一个域的结束点
                        edgedomain2=edge.getDest().identifier;//第二个域的起始点
            ...
            if (searchwaveNum == 0) {
                return result=false;//第一条路径不满足带宽请求，寻找下一条路径
                    ...
                    if (sameNum==edgeList.size()){
                        markOccupiedWavelength(searchwaveNum, bandwave, edgeList);
                        return result=true;
                    }else
    ...
    public void markOccupiedWavelength(int searchNum,int banware,List<AccessEdge> edgeList){
    ...
    public boolean subCheckUsed1(Path p, int band,int x,int edgedomain1) {
        ...
        boolean result = false; // 返回值
        //先获取第x条路径的第一条边
        int a = p.nodes.get(x).getEdgeList().get(0).getSource().identifier;
        int b = p.nodes.get(x).getEdgeList().get(0).getDest().identifier;
        //取符合第一条边带宽请求的波长编号
        for (int i=1;i<=80-bandwave+1;i++){
            searchwaveNum = oneEdgeEnough(a, b, band); //返回的波长编号
            if (searchwaveNum == 0) {
                ...
                if (sameNum==p.nodes.get(x).getEdgeList().size()){
                    markOccupiedWavelength(searchwaveNum, bandwave, p.nodes.get(x).getEdgeList());
                    return result=true;
                }else
                    unedegNum=unedegNum+1;
            }
    ...
    public int subsecondThirdWave1(int searchNum ,int bandwave,int x,Path p,int edgedomain1){
        ...
        do {
            j=j+1;
            i++;
        }while (p.nodes.get(x).getEdgeList().get(i).getDest().identifier==edgedomain1);
        for (int y = 1; y < j; y++) {
            int c = p.nodes.get(x).getEdgeList().get(y).getSource().identifier;
            int d = p.nodes.get(x).getEdgeList().get(y).getDest().identifier;
            result=istheSame(searchNum, searchNum + bandwave, c, d);
            if (result==true){
                sameNum=sameNum+1;
            }
    ...
    public int subsecondThirdWave2(int searchNum ,int bandwave,int x,Path p,int edgedomain2){
        ...
        do {
            j=j+1;
            i--;
        }while (p.nodes.get(x).getEdgeList().get(i-1).getDest().identifier==edgedomain2);
